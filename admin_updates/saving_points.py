...
class Saving_Points:
    def save_points(self, goals_player, goals, player_id, table_name):
        goals_player_dict = {}
        ...
        for j in range(1, len(goals_player)):
            if int(goals_player[0].get('week_no')) == week_no[week_index].get('week_no_id_id') and int(goals_player[j].get('player_id')) == week_no[week_index].get('player_id'):
                table_name.objects.filter(week_no_id_id = goals_player[0].get('week_no'), player_id = int(goals_player[j].get('player_id'))).update(points = goals_player[j].get('points'))
            else:
                return False
            week_index += 1
        return True

    def save_points_00(self, goals_player, goals, player_id, table_name):
        goals_player_dict = {}
        for i in range(0,len(goals)):
            goals_player_dict = {
                'player_id': player_id[i],
